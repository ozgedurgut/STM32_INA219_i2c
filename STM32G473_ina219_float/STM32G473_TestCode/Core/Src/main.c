...
float ina219_currentDivider_mA_2;
float ina219_powerMultiplier_mW_2;
uint16_t ina219_calibrationValue_2;
...
int main(void)
{
	...
	while (1)
	{
		...
		/* USER CODE BEGIN 3 */
		/*
		if (HAL_GPIO_ReadPin(GPIOA,COVERSENSOR_EXTI_Pin) == 1 ) {
			headEXTI=1;
			coverEXTI=0;
		}
		else if(HAL_GPIO_ReadPin(GPIOA,HEADSENSOR_EXTI_Pin) == 1 ){
			coverEXTI=1;
			headEXTI=0;
		}
		 */
